...
static void GifThresholdImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
    ...
    for( uint32_t ii=0; ii<numPixels; ++ii )
    {
        ...
        else
        {
            ...
            int32_t bestDiff = 1000000;
            int32_t bestInd = 1;
            GifGetClosestPaletteColor(pPal, nextFrame[PIX_R], nextFrame[PIX_G], nextFrame[PIX_B], &bestInd, &bestDiff, 1);
...
static bool GifBegin( GifWriter* writer, FILE *file, 
...
static bool GifWriteFrame( GifWriter* writer, const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay )
{
    if(!writer->f) return false;
    ...
    GifPalette pal;
    GifMakePalette(oldImage, image, width, height, 8, false, &pal);
